...
try:
	...
	f.close()
	#Italyboot
	print "Checking ItalyBoot mount path.."
	f = open('/usr/lib/enigma2/python/Plugins/Extensions/ItalyBoot/.italyboot_location', 'r')
	mypath = f.readline().strip()
	f.close()
	if not os.path.exists('/media/italyboot'):
		print "ItalyBoot mount path mounted"
		os.system("mkdir /media/italyboot && mount " + mypath + " /media/italyboot")
	else:
		print "ItalyBoot already mounted"
	#Italyboot
except:
	pass

print "Restart EMU/CS"
cmd = "italysatc '%s' '%s' '%s'" % ('127.0.0.1',PortNumber,'/etc/init.d/restartEmu.sh &')
os.system(cmd)
#End
...
from ItalySat.ItalysatSkinDev import SkinDeveloperSummary
